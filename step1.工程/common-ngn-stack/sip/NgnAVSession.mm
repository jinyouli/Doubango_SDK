...
#warning add star
//    NgnBaseService<INgnConfigurationService>* mConfigurationService = [[NgnEngine sharedInstance].configurationService retain];
//    
//    NSString * displayName = [mConfigurationService getStringWithKey:IDENTITY_DISPLAY_NAME];
//    NSString * codeStr = [self strChangeToIntChars:displayName];
//    const char *stringAsChar = [codeStr cStringUsingEncoding:[NSString defaultCStringEncoding]];
//   
//    NSString * impu = [mConfigurationService getStringWithKey:IDENTITY_IMPU];
//    const char * impuChars = [impu cStringUsingEncoding:[NSString defaultCStringEncoding]];
//    
//    SipUri * uri = new SipUri(impuChars,stringAsChar);
//    [super setFromUri:codeStr];
    //end by ZY
#warning end
...
-(NSString * )strChangeToIntChars:(NSString *)str{
    
    NSMutableString * mStr = [[NSMutableString alloc]init];
    for (int i = 0; i < str.length; i ++) {
        unichar theChar = [str characterAtIndex:i];
        
        NSString * subStr = [NSString stringWithFormat:@"%d",theChar];
...
+(BOOL) hasActiveSession{
    @synchronized (kSessions){
        NSDictionary * dic = [NgnAVSession getAllSessions];
        if (dic.count == 0) {
            return FALSE;
        }
        NSArray* values = [kSessions allValues];
...
+(NgnAVSession*) getFirstActiveCallAndNot:(long) sessionId{
    @synchronized (kSessions){
        NSMutableDictionary * dic = [NgnAVSession getAllSessions];
        if (dic.count == 0) {
            return nil;
        }
